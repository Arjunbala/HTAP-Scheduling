...
public class HeuristicTransactionOrdering implements TransactionOrdering {
	...
	// stop_time_map.put(txn,Simulation.getInstance().getTime()+
	// txn.getCPUExecutionTime());
	//
	...
	// timeEstimate =
	// Math.max(Simulation.getInstance().getTime()+t.getCPUExecutionTime(),
	// timeEstimate);
	// }
	// }
	// }
	...
	private Double getEstimateForTransactions(
		...
		while (conflictingTransactions.size() > 0) {
			...
			for (int i = 0; i < max_threads
					&& i < conflictingTransactions.size(); i++) {
				max_time = Math.max(max_time,
						conflictingTransactions.get(i).getCPUExecutionTime());
